...
/*   Updated: 2025/09/19 12:51:13 by mboutahi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void calculate_ray_direction(double x, t_player *player, double *ray_dir_x, double *ray_dir_y)
{
	double camera_x;
...
void calculate_step_and_side_dist(t_player *player, double ray_dir_x, double ray_dir_y, t_dda_data *dda)
{
	...
	{
		dda->step_x = 1;
		dda->side_dist_x = (dda->map_x + 1.0 - player->pos_x) * dda->delta_dist_x;
	}
	...
	{
		dda->step_y = 1;
		dda->side_dist_y = (dda->map_y + 1.0 - player->pos_y) * dda->delta_dist_y;
	}
}
...
void calculate_wall_distance(t_player *player, double ray_dir_x, double ray_dir_y, t_dda_data *dda)
{
	if (dda->side == 0)
		dda->perp_wall_dist = (dda->map_x - player->pos_x + (1 - dda->step_x) / 2) / ray_dir_x;
	else
		dda->perp_wall_dist = (dda->map_y - player->pos_y + (1 - dda->step_y) / 2) / ray_dir_y;
}

void calculate_draw_bounds(double perp_wall_dist, int *draw_start, int *draw_end, int *line_height)
{
...
mlx_image_t *select_texture(t_scene *scene, int side, double ray_dir_x, double ray_dir_y)
{
	mlx_image_t *tex_img;
...
void draw_solid_wall(mlx_image_t *img, int x, int draw_start, int draw_end)
{
	uint32_t wall_color;
	int y;
...
void calculate_texture_x(t_player *player, double ray_dir_x, double ray_dir_y, t_texture_data *tex_data)
{
	double wall_x;
	...
	if ((tex_data->side == 0 && ray_dir_x > 0) || (tex_data->side == 1 && ray_dir_y < 0))
		tex_data->tex_x = tex_data->tex_width - tex_data->tex_x - 1;
}

void draw_textured_wall_column(mlx_image_t *img, mlx_image_t *tex_img, int x, t_texture_data *tex_data)
{
	int y;
	int d;
	int tex_y;
	...
	y = tex_data->draw_start;
	while (y < tex_data->draw_end)
	{
		d = y * 256 - HEIGHT * 128 + tex_data->line_height * 128;
		tex_y = ((d * tex_data->tex_height) / tex_data->line_height) / 256;
		if (tex_y < 0)
			tex_y = 0;
		if (tex_y >= tex_data->tex_height)
			tex_y = tex_data->tex_height - 1;
		pixel_index = (tex_y * tex_data->tex_width + tex_data->tex_x) * 4;
		if (pixel_index >= 0 && pixel_index < (tex_data->tex_width * tex_data->tex_height * 4))
		{
			pixel = &tex_img->pixels[pixel_index];
			mlx_put_pixel(img, x, y, (pixel[0] << 24) | (pixel[1] << 16) | (pixel[2] << 8) | 0xFF);
		}
		else
...
void draw_textured_wall(mlx_image_t *img, mlx_image_t *tex_img, int x, t_wall_data *wall_data)
{
	...
	calculate_texture_x(wall_data->player, wall_data->ray_dir_x, wall_data->ray_dir_y, &tex_data);
	draw_textured_wall_column(img, tex_img, x, &tex_data);
}

void draw_single_wall_column(mlx_image_t *img, t_scene *scene, t_player *player, int x)
{
	double ray_dir_x;
	double ray_dir_y;
	t_dda_data dda;
	...
	calculate_ray_direction(x, player, &ray_dir_x, &ray_dir_y);
	calculate_step_and_side_dist(player, ray_dir_x, ray_dir_y, &dda);
	perform_dda(scene, &dda);
	calculate_wall_distance(player, ray_dir_x, ray_dir_y, &dda);
	calculate_draw_bounds(dda.perp_wall_dist, &draw_start, &draw_end, &line_height);
	tex_img = select_texture(scene, dda.side, ray_dir_x, ray_dir_y);
	if (!tex_img || !tex_img->pixels)
		draw_solid_wall(img, x, draw_start, draw_end);
	else
	{
		wall_data.player = player;
		wall_data.ray_dir_x = ray_dir_x;
		wall_data.ray_dir_y = ray_dir_y;
