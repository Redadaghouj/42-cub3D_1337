...
/*   Updated: 2025/09/26 16:37:07 by mboutahi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
mlx_image_t	*select_texture(t_scene *scene, int side, t_wall_data *wall_data)
{
	mlx_image_t	*tex_img;
									...
									int x, t_texture_data *tex_data)
{
	...
	while (tex_data->draw_start < tex_data->draw_end)
	{
		...
		pixel_index = ((int)tex_pos * tex_data->tex_width + tex_data->tex_x) * 4;
								...
								t_player *player, int x)
{
	...
	tex_img = select_texture(scene, dda.side, &wall_data);
